...
internal fun updateAppWidget(ctx: Context, appWidgetManager: AppWidgetManager, appWidgetId: Int) {
    val nAlpha = loadBgAlphaPref(ctx, appWidgetId)
    val bgColor = loadBgColorPref(ctx, appWidgetId)

    val remoteViews = RemoteViews(ctx.packageName, R.layout.memo_widget_layout)

    try {
        // 背景颜色
        val finalBgColor = Color.argb(nAlpha, Color.red(bgColor), Color.green(bgColor), Color.blue(bgColor))
        remoteViews.setInt(R.id.widget_container, "setBackgroundColor", finalBgColor)
        ...
        remoteViews.setRemoteAdapter(R.id.widget_list_view, listviewServiceIntent)
        ...
        android.util.Log.e("yelinghuye", "widget[$appWidgetId] updateAppWidget-Error: ${e.message}")
    }
}
